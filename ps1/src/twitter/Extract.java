...
public class Extract {
    ...
    public static Timespan getTimespan(List<Tweet> tweets) {
        ...
        Instant minTimespan = tweets.get(0).getTimestamp();
        Instant maxTimespan = tweets.get(0).getTimestamp();
        for (Tweet tweet : tweets) {
            if (minTimespan.isAfter(tweet.getTimestamp())) {
                minTimespan = tweet.getTimestamp();
            }
            if (maxTimespan.isBefore(tweet.getTimestamp())) {
                maxTimespan = tweet.getTimestamp();
            }
        }
        return new Timespan(minTimespan, maxTimespan);
    }
    ...
    public static Set<String> getMentionedUsers(List<Tweet> tweets) {
        ...
        for (Tweet tweet : tweets) {
            String theText = tweet.getText();
            String[] textSplit = theText.split("[^\\w@-]+");
            for (String name : textSplit) {
                if (name.matches("(@)[\\w-]+")) {
                    result.add(name.substring(1).toLowerCase());
                }
            }
        }
